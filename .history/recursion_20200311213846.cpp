...
uint16_t nussinovRecursion(const std::string & sequence, int i=0, int j=sequence.size(),uint16_t score=0)
{
    if(j <= i) 
    {
        return 0;
    ...
    if (mB !=0) {
        uint16_t m3 = nussinovRecursion(sequence,i+1,j-1,score + match(sequence,i,j));
    }
    uint16_t m4 = 0;
    for (int k = i+1; k < j;k++) 
    {
